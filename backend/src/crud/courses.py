...
def get_all_courses():
    response = (
        supabase.table("courses").select("*").order("created_at", desc=False).execute()
    )
    return response.data
...
def get_course(course_id):
    response = (
        supabase.table("courses").select("*").eq("course_id", course_id).execute()
    )
    return response.data[0] if response.data else None
...
def update_course(course_id, **kwargs):
    response = (
        supabase.table("courses").update(kwargs).eq("course_id", course_id).execute()
    )
    return response.data[0] if response.data else None
