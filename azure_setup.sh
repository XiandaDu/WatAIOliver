...
# # 3) PDF Processor Service（Internal Ingress + Dapr）
# az containerapp create -g "$RG" -n "oliver-pdf-service" --environment "$ENV" \
#   --image "${ACR}.azurecr.io/oliver-pdf-service:latest" \
#   --target-port 8003 \
#   --ingress external \
#   --enable-dapr true --dapr-app-id "oliver-pdf-service" --dapr-app-port 8003 \
#   --registry-server "${ACR}.azurecr.io" \
#   --registry-identity "$MI_RESOURCE_ID"

# # 4) Agent Service（Internal Ingress + Dapr）
# az containerapp create -g "$RG" -n "oliver-agent-service" --environment "$ENV" \
#   --image "${ACR}.azurecr.io/oliver-agent-service:latest" \
#   --target-port 8004 \
#   --ingress external \
#   --enable-dapr true --dapr-app-id "oliver-agent-service" --dapr-app-port 8004 \
#   --registry-server "${ACR}.azurecr.io" \
#   --registry-identity "$MI_RESOURCE_ID"

# # 5) Frontend（External Ingress, 静态端口）
# az containerapp create -g "$RG" -n "oliver-frontend" --environment "$ENV" \
#   --image "$PLACEHOLDER" \
#   --target-port 3000 \
#   --ingress external \
#   --registry-server "${ACR}.azurecr.io" \
