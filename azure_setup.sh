...
PLACEHOLDER="mcr.microsoft.com/mcr/hello-world:latest"

az containerapp update -g "$RG" -n "oliver-rag-service" \
  --image "$PLACEHOLDER"

az containerapp update -g "$RG" -n "oliver-backend-api" \
  --image "$PLACEHOLDER"
...
RAG_FQDN=$(az containerapp show -g "$RG" -n "oliver-rag-service" \
  --query properties.configuration.ingress.fqdn -o tsv)

API_FQDN=$(az containerapp show -g "$RG" -n "oliver-backend-api" \
  --query properties.configuration.ingress.fqdn -o tsv)

echo "RAG URL: https://${RAG_FQDN}"
